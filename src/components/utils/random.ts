...
export {}
    function getRandomNumber(min: number, max: number, isMinInclusive: boolean = true, isMaxInclusive: boolean = false): number {
        if (max < min) {
            [min, max] = [max, min];
        }
        ...
        function getRandomDate(minYear: number, maxYear: number): Date {
            ...
            const randomDay = getRandomNumber(1, new Date(randomYear, randomMonth + 1, 0).getDate(), true, true);
